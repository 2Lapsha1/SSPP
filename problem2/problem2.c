...
int main(int argc, char** argv) {
  ...
  int papi_counts = PAPI_NULL;
  long long papi_values[3];
  int event_codes[3] = {PAPI_L1_LDM, PAPI_L1_STM, PAPI_L1_DCM};

  if (PAPI_library_init(PAPI_VER_CURRENT) != PAPI_VER_CURRENT) {
    printf("PAPI init Error\n");
    return 0;
  }
  PAPI_create_eventset(&papi_counts);
  PAPI_add_events(papi_counts, event_codes, 3);

  //    PAPI_L1_DCM - data cache misses
  ...
  if (type_a == 'd') {
    ...
    for (int32_t i = 0; i < n; i++) {
      for (int32_t j = 0; j < n; j++) {
        fscanf(file_a, "%" SCNd32, &a[i][j]);
        fscanf(file_b, "%" SCNd32, &b[i][j]);
      }
    }

    PAPI_start(papi_counts);
    ...
    PAPI_stop(papi_counts, papi_values);
    ...
    for (int32_t i = 0; i < n; i++) {
      for (int32_t j = 0; j < n; j++) {
        fscanf(file_a, "%" SCNd64, &a[i][j]);
        fscanf(file_b, "%" SCNd64, &b[i][j]);
      }
    }
  ...
  printf("%lld, %lld, %lld\n", papi_values[0], papi_values[1], papi_values[2]);
